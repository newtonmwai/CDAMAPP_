...
public class WavAudioRecorder {
	...
	public static WavAudioRecorder getInstance() {
		...
		int i = 2;
		do {
			result = new WavAudioRecorder(AudioSource.MIC, sampleRates[i],
	...
	private int aFormat;
	// Number of frames/samples written to file on each output(only in
	// uncompressed mode)
	...
	/**
	 *
	 * Returns the state of the recorder in a WavAudioRecorder.State typed
	 ...
	 * Default constructor
	 *
	 * Instantiates a new recorder In case of errors, no exception is thrown,
	 * but the state is set to ERROR
	 *
	 */
	public WavAudioRecorder(int audioSource, int sampleRate, int channelConfig,
			int audioFormat) {
		try {
			...
			if (mBufferSize < AudioRecord.getMinBufferSize(sampleRate,
				...
				mPeriodInFrames = mBufferSize
						/ (2 * mBitsPersample * nChannels / 8);
			...
			if (state == State.INITIALIZING) {
				if ((audioRecorder.getState() == AudioRecord.STATE_INITIALIZED)
					...
					randomAccessWriter.writeInt(Integer.reverseBytes(sRate
							* nChannels * mBitsPersample / 8)); // Byte rate,
					...
					randomAccessWriter.writeShort(Short
							.reverseBytes(mBitsPersample)); // Bits per sample
					randomAccessWriter.writeBytes("data");
					randomAccessWriter.writeInt(0); // Data chunk size not known
													// yet, write 0
					buffer = new byte[mPeriodInFrames * mBitsPersample / 8
							* nChannels];
					state = State.READY;
				} else {
					Log.e(WavAudioRecorder.class.getName(),
	...
	public void reset() {
		try {
			if (state != State.ERROR) {
				...
				audioRecorder = new AudioRecord(mAudioSource, sRate, nChannels,
						aFormat, mBufferSize);
				if (audioRecorder.getState() != AudioRecord.STATE_INITIALIZED) {
					throw new Exception("AudioRecord initialization failed");
				}
